...
# Глобальные переменные для управления отложенным сном
...
def create_timer_image(minutes):
    """Создаем увеличенное изображение с таймером для иконки в трее"""
    width, height = 128, 128  # Увеличен размер
    ...
    try:
        font = ImageFont.truetype("arial.ttf", 48)  # Крупный шрифт
    except:
...
def countdown(seconds, tray):
    """Обратный отсчет с обновлением иконки и подсказки в трее"""
    ...
    try:
        while True:
            try:
                menu_item = tray.read()
                if menu_item == 'Exit':
                    # При выходе отменяем отложенный сон, если он активен
        ...
        # Удаляем временный файл иконки при выходе
        try:
            os.unlink('image.png')
        except:
            pass
